...
def make_recipes() -> list[Recipe]:
    return [
        ...
        _recipe(recipe_id="dinner_1", name="Pasta", meals=["dinner"], prep=10, cook=20, servings=2),
        _recipe(recipe_id="dinner_2", name="Stir Fry", meals=["dinner"], prep=15, cook=15, servings=2, yields=["veggies"]),
...
def make_config() -> WeekConfig:
    config_dict = {
        ...
        "max_daily_cook_times": {day: {meal: 60 for meal in ["breakfast", "lunch", "dinner", "snack"]} for day in [
            "Sunday",
            "Monday",
            "Tuesday",
            "Wednesday",
...
def test_generate_plan_respects_history_and_skips():
    ...
    assert "soup_stock" in summary["yielded_prep_items"]
